...
**Zero-config connection via env var (recommended — pinned, but not verified before it runs).** The repo commits a [`.mcp.json`](.mcp.json) that launches `hostinger-api-mcp` through `npx` (version-pinned, no global install needed; `npx` fetches and executes in one step on every run, so the bytes cannot be checked first — the manual path below is the verified one) with the token from the `${HOSTINGER_API_TOKEN:-}` env placeholder — set the variable in your shell (devices) or in the claude.ai cloud environment's env vars (web/phone sessions) and the connection authenticates on its own. While the variable is unset the connection just shows as unavailable in `/mcp` (it can't authenticate) — set the var to bring it up. Set `HOSTINGER_MCP_BINARY` (e.g. `hostinger-vps-mcp`) to load a single category binary instead of all 127 tools. Real tokens never go into the file — it is tracked in git. **Migrating from a local gitignored `.mcp.json`:** move your token to the env var (or `claude mcp add -s user`), delete the local file, then pull.

**Manual per-category connections (the verified path).** Do **not** run a bare `npm i -g`: installing an npm package runs its `preinstall` / `install` / `postinstall` scripts, so whatever the registry serves executes before you have looked at it — into a process that then holds a full-account token. Follow [Step 1 of `installation.md`](.claude/skills/hostinger-mcp/references/installation.md#step-1--install), which fetches with `npm pack`, compares against the digest recorded there and **exits non-zero on a mismatch with nothing installed**, then installs from that verified local tarball. Then wire user-scope connections per category/account with `claude mcp add -s user`, passing the token as a quoted `${VAR}` placeholder rather than typing its value into the command — see [`installation.md`](.claude/skills/hostinger-mcp/references/installation.md) and [`.mcp.json.example`](.mcp.json.example).
